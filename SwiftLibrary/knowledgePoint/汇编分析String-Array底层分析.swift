...
func mainFunc() {
    ...
    let arr2 = ["A","B"]
    ///有多少个元素就会调用多少次这个闭包表达式
    var arr3 = arr2.map { (element) -> String in
        
        return element + "A"
    }
    //会把返回的数组类型平铺开来，map不会 SegmentOfResult  返回值遵守Sequence协议
    
//    let arr4 = arr2.flatMap { (elemet) -> Array<String> in
//        return Array.init(repeating: "3", count: 3)
//    }
    let arr4 = [nil,Optional("A"),Optional("B")].flatMap { (elemet) -> String? in
        
        return elemet
    }
    //lazy 用到的时候才会调用大括号里面的 arr5[0]这样才会调用一次这个闭包传入对应的元素
    let arr5 = [nil,Optional("A"),Optional("B")].lazy.compactMap { (elemet) -> String? in
    ...
    ///return true就代表是需要的
    arr3 = arr2.filter { (element) -> Bool in
        return element == "A" ? true : false
    }
    
    print(arr3)
//    func reduce<Result>(_ initialResult: Result, _ nextPartialResult: (Result, Self.Element) throws -> Result) rethrows -> Result
    /**
     @pragma result：上次便利的元素
     @pragma element：数组的元素
     @pragma ""：初始值
     */
    let str = arr2.reduce("C") { (result, element) -> String in
