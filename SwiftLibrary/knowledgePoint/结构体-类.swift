...
/**
 ...
 /// 5.堆空间前面有16个字节是有用的 前8个是类信息  用计数  然后后面才是存储成员变量的内存
    类型信息：指向代码区该类型的方法列表
 /// 6.枚举和结构体 类里面都可以定义方法
...
///kvc kvo
class KVC_KVO: NSObject {
    
    @objc dynamic var name:String = "123"
    //第二种kvo的方式
    var observtation:NSKeyValueObservation? = nil
    
    override init() {
        super.init()
        observtation = observe(\KVC_KVO.name, options: [NSKeyValueObservingOptions.new ,.old]) {
    ...
    deinit {
        //如果使用上面的这种observe就不需要移除的了
//        removeObserver(self, forKeyPath: "name")
    }
    
    
    
//    observe(\KVC_KVO.name) { (kvc_kvo, change) in
//        print(change.)
//    }
}
/**
 属性所在的类，监听器都必须要继承自nsobject
 用 @objc dynamic修饰该属性
 */
...
func testKVC_KVO()  {
    ...
    let kk = KVC_KVO()
//    let obs = obsever()
//    kk.addObserver(obs, forKeyPath: "name", options: NSKeyValueObservingOptions.new, context: nil)
    kk.setValue("测试", forKey: "name")
//    print(kk.name)
}
//入口
class struct_class{
